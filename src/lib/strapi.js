...
/**
 * GET כללי ל-Strapi - תומך ב-pagination/sort/filters/populate דרך params (אובייקט שטוח).
 * דוגמה: strapiGet('pg-campaigns', { sort: 'order:asc' })
 */
...
/**
 * POST כללי ל-Strapi - לטופסי משוב וכו'.
 * Strapi 5 דורש שהגוף יהיה { data: {...} }.
 */
export async function strapiPost(path, data, { fetch: f = fetch } = {}) {
    ...
    if (!res.ok) {
        throw new Error(`Strapi POST ${path} failed: ${res.status} ${res.statusText}`);
    }
    return res.json();
}

// ----- API עזר ספציפי לקמפיינים -----

export async function fetchCampaigns({ fetch: f = fetch } = {}) {
    // Strapi 5: 'sort' תואם sort=order:asc; 'pagination[pageSize]' להחזרת רבים
...
export async function fetchStat({ fetch: f = fetch } = {}) {
    const data = await strapiGet('pg-stat', {}, { fetch: f });
